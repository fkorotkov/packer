...
//
// Errors if no build can be found or if the hcp names map is uninitialized
func (cfg *PackerConfig) HCPName(buildName string) (string, error) {
	if cfg.HCPBuildNames == nil {
		return "", fmt.Errorf("uninitialized hcp names")
	}

	nm, ok := cfg.HCPBuildNames[buildName]
	if !ok {
